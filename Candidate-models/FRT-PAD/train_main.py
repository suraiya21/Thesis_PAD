...
def load_net_datasets():
    ...
    net.cuda()
    #net.load_state_dict(torch.load('Train_mobai_train_test_mobai_test_8_HTER_0.0.pth'))
    return net, train_data_loader, test_data_loader

#@jit(target_backend='cuda')
def train():
    net, train_loader, test_loader = load_net_datasets()
    #net.load_state_dict(torch.load('save_models/FR_direct_Graph_WMCA/Train_mobai_train_test_mobai_test_10_HTER_0.0.pth'))
    ...
    for e in range(config.Epoch_num):
        ...
        for b, (imgs, labels) in enumerate(t):
            imgs = imgs.cuda()
