...
def oulu_process(data_dir, label_save_dir):
    ...
    dataset_path = data_dir
    #print(dataset_path)
    path_list = glob.glob(dataset_path + '**/*/*.jpg', recursive=True) + glob.glob(dataset_path + '**/*/*.png', recursive=True) + glob.glob(dataset_path + '**/*/*.bmp', recursive=True) 
    print(len(path_list))
    path_list.sort()
    ...
    for i in range(len(path_list)):
        #row = []
        #print(path_list[i])
        flag = path_list[i].find('1_bonafide_crop_rgb') 
        ...
        if(path_list[i].find('/2_4_CASIA_Anti/') != -1):

            Domain_1.writerow(dict)
 
        elif(path_list[i].find('/4_6_oulu/') != -1):
