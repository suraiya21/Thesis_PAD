...
class ViT_AvgPool_3modal_CrossAtten_Channel(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        #self.mixstyle = MixStyle(p=0.5, alpha=0.1, mix="crossdomain")
    ...
    def forward(self, x1, x2):
        ...
        #h_concat = self.mixstyle(h_concat)
        
        regmap8 =  self.avgpool8(h_concat)
        #print(self.drop(regmap8.squeeze(-1).squeeze(-1)).size())
        
        logits = self.fc(self.drop(regmap8.squeeze(-1).squeeze(-1)))
        #print(regmap8.squeeze(-1).squeeze(-1))
        #print(logits)
